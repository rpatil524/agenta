...
[Evaluations](/reference/api-guide/evaluations) are configured with one or more evaluators that run against a testset. Each evaluation pins specific `evaluator_revision_id`s when the run is configured. Committing a new revision on the variant does not retroactively change a pinned run.
