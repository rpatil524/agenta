...
[Evaluations](/reference/api-guide/evaluations) are run against a testset. Each evaluation pins a specific `testset_revision_id` when the run is configured. Committing a new revision on the testset does not retroactively change a pinned run.
