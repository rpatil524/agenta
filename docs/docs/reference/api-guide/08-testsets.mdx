...
A testcase can carry a caller-supplied dedup ID. The server uses it to identify the same row across re-imports and edits.
...
Both are accepted on write. The server normalises `__dedup_id__` into `data.testcase_dedup_id` on its way in. On export (CSV or JSON via `/download`), the server emits the legacy `__dedup_id__` form for round-trip compatibility with tools that expect it.

If a row has no dedup ID on write, the server generates one from the content hash.
...
An evaluation pins a specific **testset revision ID** when the run is configured. Committing a new revision on the testset does not retroactively change a pinned run. See [Evaluations](/reference/api-guide/evaluations).
