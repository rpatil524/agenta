...
Agenta's observability sits on top of OpenTelemetry. This page covers the span and trace model, the `ag.*` attribute namespace Agenta adds on top, references that link spans to Agenta entities, the four ingestion endpoints, and querying.
...
|----------|-------------|
| [`POST /otlp/v1/traces`](/reference/api/otlp-ingest) | Raw OTLP protobuf from an OTel SDK or collector. Use this when your existing OTel instrumentation can just point its exporter at Agenta. |
| [`POST /tracing/spans/ingest`](/reference/api/ingest-spans) | Agenta-native structured ingest. Accepts either a flat `spans` list (with `parent_id` linking) or a nested `traces` tree. Use this when you're writing spans yourself with full control over IDs and hierarchy. |
...
| [`POST /traces/ingest`](/reference/api/ingest-traces), [`POST /traces/`](/reference/api/create-trace), [`PUT /traces/{trace_id}`](/reference/api/edit-trace) | List-shaped trace CRUD. Accepts the canonical `Trace` and `Traces` shape. Use this when operating on whole traces as records. |

### How ingest works

Every ingest endpoint returns `202 Accepted` *before* the spans are persisted. The router parses the payload, hands the spans off to an async worker, and responds immediately. OTel semantics require the ingest buffer to return fast, so instrumented apps do not block on telemetry.
...
The adapter that parses incoming spans into Agenta's attribute schema is lenient. When a field doesn't fit, the adapter moves it under `ag.unsupported.*` on the span rather than reject the span outright. See [`ag.unsupported`](#agunsupported) for what lands there.
...
Two endpoints return spans, with different shapes:

- [`POST /tracing/traces/query`](/reference/api/query-traces) returns spans grouped into a nested tree by `trace_id`. This is the same shape that `tracing/spans/ingest` accepts under `traces`.
- [`POST /tracing/spans/query`](/reference/api/query-spans) returns a flat list of spans with cursor pagination.
